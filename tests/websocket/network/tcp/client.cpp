...
ut::suite websocket_network_tcp_client = [] {
  ...
  "parallel read and close completes without deadlock"_test = [] {
    ...
    expect(fatal(close_future.wait_for(6s) == std::future_status::ready));
    auto close_ec = close_future.get();
    expect(not close_ec);

    expect(fatal(read_future.wait_for(6s) == std::future_status::ready));
    ...
    if (read_result) {
      ...
      expect(read_result.error() == protocol_error::connection_closed or read_result.error() == asio::error::operation_aborted);
    }
  ...
  "parallel read and concurrent sends return all echoes"_test = [] {
...

    {
      ...
      expect(std::ranges::all_of(expected_payloads, [&](const std::string& text) { return received_payloads.contains(text); }));
    }
