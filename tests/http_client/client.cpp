...
int main() {
  suite http_client = [] {
    ...
    "url overload parses target and reuses connection"_test = [] {
      ...
      auto second =
        send_with_timeout(client, make_local_endpoint(server.port()), make_request("/second"), local_request_timeout);
    ...
    "retries idempotent request after stale persistent connection closes"_test = [] {
      ...
      expect[first_connection_aborted.wait_for(coordination_timeout) == std::future_status::ready];
