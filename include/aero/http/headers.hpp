...
namespace aero::http {
  ...
  class headers {
   ...
   private:
    ...
    class matching_iterator {
      using owner_type = std::conditional_t<IsConst, const http::headers*, http::headers*>;

     public:
      using iterator_concept = std::bidirectional_iterator_tag;
      using iterator_category = std::bidirectional_iterator_tag;
      using difference_type = ptrdiff_t;
      using value_type = http::headers::value_type;
     ...
     private:
      ...
      void seek_backward() noexcept {
        while (index_ > 0) {
          --index_;
          if (aero::striequal(owner_->headers_[index_].name, key_)) {
            return;
          }
        }
      }
   ...
   public:
    ...
    [[nodiscard]] auto values(std::string_view name AERO_LIFETIMEBOUND) const& {
      return headers_ | std::views::filter([name](const http::header& field) { return aero::striequal(field.name, name); }) |
             std::views::transform([](const http::header& field) noexcept -> std::string_view { return field.value; });
    }
  ...
  static_assert(std::ranges::bidirectional_range<decltype(std::declval<headers&>().fields(""))>);
  static_assert(std::ranges::bidirectional_range<decltype(std::declval<headers&>().names())>);
  static_assert(std::ranges::bidirectional_range<decltype(std::declval<headers&>().values(""))>);
